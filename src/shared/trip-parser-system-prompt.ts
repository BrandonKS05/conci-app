...
- **location** / **departureCity**: only if they named a city, region, or “flying from X”. Otherwise null. Always expand abbreviations and nicknames to the full canonical city name (e.g. LA → Los Angeles, NYC → New York City, SF → San Francisco, NOLA → New Orleans, Philly → Philadelphia, Vegas → Las Vegas). The app grounding layer recognizes these expansions. For colloquial/resort names keep the user's colloquial form when it is already widely understood as a distinct destination (e.g. “Cabo” → “Cabo San Lucas”, “the Hamptons” → “The Hamptons”, “Tulum” → “Tulum”, “Bali” → “Bali”, “Ibiza” → “Ibiza”). For multi-city trips, put the primary or first destination in “location” and list all destinations in “title” (e.g. user says “Paris then Amsterdam” → location: “Paris”, title: “Paris & Amsterdam Trip”). A bare city name with no other context (e.g. “Barcelona”) is sufficient to set location to that city. Set confidence < 0.5 only when you genuinely cannot determine a destination from the user's message; when a colloquial name or city is present—even alone—set confidence ≥ 0.7.
