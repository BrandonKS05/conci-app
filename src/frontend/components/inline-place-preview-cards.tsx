...
function PlaceCard({ item }: { item: PlacePreview }) {
  return (
    <a
      ...
      className="flex max-w-full overflow-hidden rounded-xl border border-[color:var(--hairline)] bg-white text-left shadow-sm ring-1 ring-slate-200/60 transition hover:border-orange-400/40 hover:ring-orange-400/20 dark:border-white/10 dark:bg-[#1e1e1e] dark:ring-white/[0.04] dark:hover:border-[#ea580c]/40"
    >
      {item.photoUrl ? (
        <Image
          src={item.photoUrl}
