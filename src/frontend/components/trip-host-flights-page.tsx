  ...
  if (flight.airlineLogoUrl && !logoFailed) {
    return (
      ...
      <img
        ...
        className="h-10 w-10 rounded-md border border-[color:var(--hairline)] object-cover dark:border-white/15"
        onError={() => setLogoFailed(true)}
      />
    );
  }
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-md border border-[color:var(--hairline)] bg-[color:var(--surface-container)] text-xs font-semibold text-[color:var(--on-surface-variant)] dark:border-white/15 dark:bg-white/5 dark:text-neutral-200">
...
export function TripHostFlightsPage(props: {
  ...
  return (
    <div className="min-h-screen bg-[color:var(--surface-container-low)] py-6 text-[color:var(--on-surface)] dark:bg-dm-page dark:text-neutral-100 sm:py-8">
      <div className="mx-auto w-full max-w-6xl px-4">
        <Link
          href={`/trip/${props.tripId}/setup`}
          className="inline-flex items-center text-sm font-medium text-[color:var(--on-surface-variant)] underline-offset-2 hover:underline dark:text-neutral-400"
        >
          ← Back to trip setup
        </Link>

        <h1 className="mt-4 text-2xl font-bold tracking-tight text-[color:var(--on-surface)] dark:text-white">{header}</h1>
              ...
              >
                <div className="grid gap-3 px-4 py-3 md:grid-cols-[minmax(0,1.55fr)_minmax(0,1fr)_auto] md:items-start">
                  ...
                  <div className="text-right md:min-w-[132px]">
                    <p className="text-[34px] font-bold leading-none tracking-tight text-[color:var(--on-surface)] dark:text-white">{formatPrice(f.price)}</p>
        ...
        {totalPages > 1 ? (
          <div className="mt-4 flex items-center justify-between text-sm text-[color:var(--on-surface-variant)] dark:text-neutral-400">
            <button
              type="button"
              disabled={page <= 1}
