...
import type { ItineraryActionRequest, ItineraryItem } from "@/lib/itinerary-model";
import type { ItineraryScreenData, ItinerarySection } from "@/lib/itinerary-view";
import { mutateActiveItinerary } from "@/lib/itinerary-api";
  ...
  return (
    <div className="space-y-6">
      <GlassCard className="p-5 sm:p-6">
        ...
        <div className="mt-5 flex flex-col gap-3 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-end">
          <label className="grid gap-1">
            ...
            <input
              ...
              inputMode="numeric"
              className="w-32 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-ink outline-none focus:border-brand-500"
            />
          </label>
          <button
            type="button"
            ...
            className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-ink transition hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
            Update guests
          </button>
      ...
      <GlassCard className="p-5 sm:p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <SecondaryButton href="/saved">Open saved itinerary</SecondaryButton>
        </div>
      </GlassCard>
        ...
        <div className="grid gap-3">
          {data.itinerary.selections.length > 0 ? (
            data.itinerary.selections.slice(-4).map((selection) => (
              <div key={selection.id} className="rounded-[1.35rem] border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                <p className="font-semibold text-ink">{selection.status.replaceAll("_", " ")}</p>
                <p className="mt-1">{selection.created_at}</p>
              </div>
            ))
          ) : (
            <p className="text-sm leading-6 text-slate-600">No changes yet.</p>
          )}
        </div>
      </GlassCard>
    </div>
...
function ItinerarySectionCard({
  ...
  return (
    <GlassCard className="p-5 sm:p-6">
      ...
      <div className="mt-4 rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
        ...
        <h4 className="mt-4 font-display text-xl font-semibold tracking-[-0.03em] text-ink">
          {item.title}
        </h4>
        <p className="mt-2 text-sm text-slate-500">{item.details}</p>
        <p className="mt-4 text-sm leading-6 text-slate-600">{item.meta}</p>
        <p className="mt-4 text-sm font-semibold text-ink">{item.price}</p>
        ...
        {isEditing ? (
          <div className="mt-5 grid gap-3 rounded-[1.25rem] border border-slate-200 bg-slate-50 p-4">
      ...
      {section.alternatives.length > 0 ? (
        <div className="mt-5">
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-500">Alternatives</p>
          <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {section.alternatives.map((alt) => (
              <div key={alt.id} className="rounded-[1.4rem] border border-slate-200 bg-slate-50 p-4">
                <p className="font-display text-lg font-semibold tracking-[-0.03em] text-ink">{alt.title}</p>
                <p className="mt-1 text-sm text-slate-500">{alt.details}</p>
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => onSelectAlternative(alt)}
                  className="mt-4 inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-ink transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Swap in this option
                </button>
              </div>
...
function StepButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-ink transition hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
    >
      {label}
    </button>
  );
  ...
  return (
    <label className="grid gap-1">
      ...
      <input
        ...
        className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-ink outline-none focus:border-brand-500"
      />
    </label>
  );
}
